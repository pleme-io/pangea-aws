...
# Resource: Manages a aws_scheduler_schedule_group resource
# Source: terraform-apply

require 'spec_helper'

RSpec.describe Pangea::Resources::AWSSchedulerScheduleGroup do
  include Pangea::Testing::SynthesisTestHelpers

  let(:required_attrs) { {} }

  describe ':aws_scheduler_schedule_group' do
    context 'with required attributes only' do
      it 'synthesizes valid Terraform JSON' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_scheduler_schedule_group('test', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        validate_terraform_structure(result, :resource)
        config = validate_resource_structure(result, 'aws_scheduler_schedule_group', 'test')
      ...
      it 'returns a ResourceReference' do
        synth = create_synthesizer
        synth.extend(described_class)
        ref = synth.aws_scheduler_schedule_group('test', required_attrs)

        expect(ref).to be_a(Pangea::Resources::ResourceReference)
        expect(ref.resource_type).to eq(:aws_scheduler_schedule_group)
      end
      ...
      it 'provides output references' do
        ...
        expect(ref.id).to eq("${aws_scheduler_schedule_group.test.id}")
        expect(ref.arn).to eq("${aws_scheduler_schedule_group.test.arn}")
        expect(ref.creation_date).to eq("${aws_scheduler_schedule_group.test.creation_date}")
    ...
    context 'with all attributes' do
      ...
      it 'synthesizes with optional attributes' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_scheduler_schedule_group('full', all_attrs)
        result = normalize_synthesis(synth.synthesis)
    ...
    context 'optional attributes' do
      ...
      it 'includes tags when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_scheduler_schedule_group('opt', required_attrs.merge(tags: { 'key1' => 'val1' }))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'aws_scheduler_schedule_group', 'opt')
        expect(config).to have_key('tags')
      end
    ...
    context 'attribute types' do
      it 'validates expected attribute types' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_scheduler_schedule_group('typed', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        config = validate_resource_structure(result, 'aws_scheduler_schedule_group', 'typed')
      end
    end
    ...
    context 'multiple instances' do
      it 'synthesizes multiple resources independently' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_scheduler_schedule_group('first', required_attrs)
        synth.aws_scheduler_schedule_group('second', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        resources = result.dig('resource', 'aws_scheduler_schedule_group')
