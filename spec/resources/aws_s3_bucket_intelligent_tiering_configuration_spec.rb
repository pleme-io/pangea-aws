...
# Resource: Manages a aws_s3_bucket_intelligent_tiering_configuration resource
# Source: terraform-apply

require 'spec_helper'

RSpec.describe Pangea::Resources::AWSS3BucketIntelligentTieringConfiguration do
  include Pangea::Testing::SynthesisTestHelpers

  let(:required_attrs) { { bucket: 'test-value', name: 'test-value', tiering: [{ 'key1' => 'val1' }] } }

  describe ':aws_s3_bucket_intelligent_tiering_configuration' do
    context 'with required attributes only' do
      it 'synthesizes valid Terraform JSON' do
        ...
        validate_required_attributes(config, [:bucket, :name, :tiering])
      end
      ...
      it 'returns a ResourceReference' do
        ...
        expect(ref.resource_type).to eq(:aws_s3_bucket_intelligent_tiering_configuration)
      end
    ...
    context 'with all attributes' do
      ...
      it 'synthesizes with optional attributes' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_s3_bucket_intelligent_tiering_configuration('full', all_attrs)
        result = normalize_synthesis(synth.synthesis)
    ...
    context 'optional attributes' do
      ...
      it 'includes status when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_s3_bucket_intelligent_tiering_configuration('opt', required_attrs.merge(status: 'test-value'))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'aws_s3_bucket_intelligent_tiering_configuration', 'opt')
        expect(config).to have_key('status')
      end
    ...
    context 'attribute types' do
      it 'validates expected attribute types' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_s3_bucket_intelligent_tiering_configuration('typed', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        config = validate_resource_structure(result, 'aws_s3_bucket_intelligent_tiering_configuration', 'typed')
        expect(config['bucket']).to be_a(String)
        expect(config['name']).to be_a(String)
        expect(config['tiering']).to be_a(Array)
      end
    end
    ...
    context 'multiple instances' do
      it 'synthesizes multiple resources independently' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_s3_bucket_intelligent_tiering_configuration('first', required_attrs)
        synth.aws_s3_bucket_intelligent_tiering_configuration('second', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        resources = result.dig('resource', 'aws_s3_bucket_intelligent_tiering_configuration')
