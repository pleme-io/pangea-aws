...
# Resource: Manages a aws_quicksight_folder_membership resource
# Source: terraform-apply

require 'spec_helper'

RSpec.describe Pangea::Resources::AWSQuicksightFolderMembership do
  include Pangea::Testing::SynthesisTestHelpers

  let(:required_attrs) { { folder_id: 'test-value', member_id: 'test-value', member_type: 'test-value' } }

  describe ':aws_quicksight_folder_membership' do
    context 'with required attributes only' do
      it 'synthesizes valid Terraform JSON' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_quicksight_folder_membership('test', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        validate_terraform_structure(result, :resource)
        config = validate_resource_structure(result, 'aws_quicksight_folder_membership', 'test')
        validate_required_attributes(config, [:folder_id, :member_id, :member_type])
      end

      it 'returns a ResourceReference' do
        synth = create_synthesizer
        synth.extend(described_class)
        ref = synth.aws_quicksight_folder_membership('test', required_attrs)

        expect(ref).to be_a(Pangea::Resources::ResourceReference)
        expect(ref.resource_type).to eq(:aws_quicksight_folder_membership)
      end
    ...
    context 'attribute types' do
      it 'validates expected attribute types' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_quicksight_folder_membership('typed', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        config = validate_resource_structure(result, 'aws_quicksight_folder_membership', 'typed')
        expect(config['folder_id']).to be_a(String)
        expect(config['member_id']).to be_a(String)
        expect(config['member_type']).to be_a(String)
      end
    end
    ...
    context 'multiple instances' do
      it 'synthesizes multiple resources independently' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_quicksight_folder_membership('first', required_attrs)
        synth.aws_quicksight_folder_membership('second', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        resources = result.dig('resource', 'aws_quicksight_folder_membership')
