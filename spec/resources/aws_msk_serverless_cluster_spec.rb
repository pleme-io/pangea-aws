...
# Resource: Manages a aws_msk_serverless_cluster resource
  ...
  describe ':aws_msk_serverless_cluster' do
    context 'with required attributes only' do
      it 'synthesizes valid Terraform JSON' do
        ...
        synth.aws_msk_serverless_cluster('test', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        validate_terraform_structure(result, :resource)
        config = validate_resource_structure(result, 'aws_msk_serverless_cluster', 'test')
        validate_required_attributes(config, [:client_authentication, :cluster_name, :vpc_config])
      end

      it 'returns a ResourceReference' do
        synth = create_synthesizer
        synth.extend(described_class)
        ref = synth.aws_msk_serverless_cluster('test', required_attrs)

        expect(ref).to be_a(Pangea::Resources::ResourceReference)
        expect(ref.resource_type).to eq(:aws_msk_serverless_cluster)
      end
      ...
      it 'provides output references' do
        ...
        expect(ref.id).to eq("${aws_msk_serverless_cluster.test.id}")
    ...
    context 'with all attributes' do
      ...
      it 'synthesizes with optional attributes' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_msk_serverless_cluster('full', all_attrs)
        result = normalize_synthesis(synth.synthesis)
    ...
    context 'optional attributes' do
      ...
      it 'includes tags when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_msk_serverless_cluster('opt', required_attrs.merge(tags: { 'key1' => 'val1' }))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'aws_msk_serverless_cluster', 'opt')
        expect(config).to have_key('tags')
      end
    ...
    context 'attribute types' do
      it 'validates expected attribute types' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_msk_serverless_cluster('typed', required_attrs)
        result = normalize_synthesis(synth.synthesis)
    ...
    context 'multiple instances' do
      it 'synthesizes multiple resources independently' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_msk_serverless_cluster('first', required_attrs)
        synth.aws_msk_serverless_cluster('second', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        resources = result.dig('resource', 'aws_msk_serverless_cluster')
