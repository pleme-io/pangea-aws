...
# Resource: Manages a aws_cognito_identity_provider resource
# Source: terraform-apply

require 'spec_helper'

RSpec.describe Pangea::Resources::AWSCognitoIdentityProvider do
  include Pangea::Testing::SynthesisTestHelpers

  let(:required_attrs) { { provider_details: { 'key1' => 'val1' }, provider_name: 'test-value', provider_type: 'test-value', user_pool_id: 'test-value' } }

  describe ':aws_cognito_identity_provider' do
    context 'with required attributes only' do
      it 'synthesizes valid Terraform JSON' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_cognito_identity_provider('test', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        validate_terraform_structure(result, :resource)
        config = validate_resource_structure(result, 'aws_cognito_identity_provider', 'test')
        validate_required_attributes(config, [:provider_details, :provider_name, :provider_type, :user_pool_id])
      end

      it 'returns a ResourceReference' do
        synth = create_synthesizer
        synth.extend(described_class)
        ref = synth.aws_cognito_identity_provider('test', required_attrs)

        expect(ref).to be_a(Pangea::Resources::ResourceReference)
        expect(ref.resource_type).to eq(:aws_cognito_identity_provider)
      end
    ...
    context 'with all attributes' do
      ...
      it 'synthesizes with optional attributes' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_cognito_identity_provider('full', all_attrs)
        result = normalize_synthesis(synth.synthesis)
    ...
    context 'optional attributes' do
      ...
      it 'includes idp_identifiers when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_cognito_identity_provider('opt', required_attrs.merge(idp_identifiers: ['test-value']))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'aws_cognito_identity_provider', 'opt')
        expect(config).to have_key('idp_identifiers')
      end

      it 'omits idp_identifiers when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_cognito_identity_provider('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
    ...
    context 'attribute types' do
      it 'validates expected attribute types' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_cognito_identity_provider('typed', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        config = validate_resource_structure(result, 'aws_cognito_identity_provider', 'typed')
        expect(config['provider_details']).to be_a(Hash)
    ...
    context 'multiple instances' do
      it 'synthesizes multiple resources independently' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.aws_cognito_identity_provider('first', required_attrs)
        synth.aws_cognito_identity_provider('second', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        resources = result.dig('resource', 'aws_cognito_identity_provider')
