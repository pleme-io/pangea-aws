...
RSpec.describe 'aws_lb_target_group_attachment synthesis' do
  ...
  describe 'terraform synthesis' do
    it 'synthesizes with tags' do
      synthesizer.instance_eval do
        ...
        aws_lb_target_group_attachment(:tagged, {
          tags: { Environment: 'production' }
        })
      end

      result = synthesizer.synthesis
      attachment = result[:resource][:aws_lb_target_group_attachment][:tagged]
  ...
  describe 'resource references' do
    it 'returns ResourceReference with correct outputs' do
      ...
      expect(ref.outputs[:id]).to eq('${aws_lb_target_group_attachment.test.id}')
    end
  end
end
