# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.

require 'pangea/resources/base'
require 'pangea/resources/reference'
require 'pangea/resources/aws_opensearch_outbound_connection/types'
require 'pangea/resource_registry'

module Pangea::Resources
  module AWSOpensearchOutboundConnection
    include Pangea::Resources::ResourceBuilder

    define_resource :aws_opensearch_outbound_connection,
      attributes_class: AWS::Types::OpensearchOutboundConnectionAttributes,
      outputs: { id: :id },
      map: [:connection_alias, :local_domain_info, :remote_domain_info],
      map_present: [:connection_properties],
      map_bool: [:accept_connection]
  end
  module AWS
    include AWSOpensearchOutboundConnection
  end
end
Pangea::ResourceRegistry.register_module(Pangea::Resources::AWS)
