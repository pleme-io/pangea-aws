          ...
          module Statements
            ...
            def build_byte_match(ctx, config)
              ...
              ctx.byte_match_statement do
                ctx.positional_constraint config[:positional_constraint]
                ctx.search_string config[:search_string]
                ctx.field_to_match { builder.build_field_to_match(ctx, config[:field_to_match]) }
                builder.build_text_transformations(ctx, config[:text_transformations])
              end
            end
            ...
            def build_size_constraint(ctx, config)
              ...
              ctx.size_constraint_statement do
                ctx.comparison_operator config[:comparison_operator]
            ...
            def build_geo_match(ctx, config)
              ctx.geo_match_statement do
                ctx.country_codes config[:country_codes]
                next unless config[:forwarded_ip_config]

                ctx.forwarded_ip_config do
                  ctx.header_name config[:forwarded_ip_config][:header_name]
                  ctx.fallback_behavior config[:forwarded_ip_config][:fallback_behavior]
                end
              end
            end
            ...
            def build_ip_set_reference(ctx, config)
              ctx.ip_set_reference_statement do
                ...
                ctx.ip_set_forwarded_ip_config do
                  ctx.header_name config[:ip_set_forwarded_ip_config][:header_name]
                  ctx.fallback_behavior config[:ip_set_forwarded_ip_config][:fallback_behavior]
                  ctx.position config[:ip_set_forwarded_ip_config][:position]
                end
              end
            end
            ...
            def build_rule_group_reference(ctx, config)
              ctx.rule_group_reference_statement do
                ctx.arn config[:arn]
                config[:excluded_rules]&.each do |rule|
                  ctx.excluded_rule do
                    ctx.name rule[:name]
                  end
                end
              end
            ...
            def build_managed_rule_group(ctx, config)
              ...
              ctx.managed_rule_group_statement do
                ctx.vendor_name config[:vendor_name]
                ctx.name config[:name]
                ctx.version config[:version] if config[:version]
                config[:excluded_rules]&.each do |rule|
                  ctx.excluded_rule do
                    ctx.name rule[:name]
                  end
                ...
                ctx.scope_down_statement { builder.build_statement(ctx, config[:scope_down_statement]) }
              end
            end
            ...
            def build_rate_based(ctx, config)
              ...
              ctx.rate_based_statement do
                ...
                ctx.aggregate_key_type config[:aggregate_key_type]
            ...
            def build_logical_statement(ctx, type, statements)
              builder = self
              ctx.public_send(type) do
                statements.each do |sub|
                  ctx.statement { builder.build_statement(ctx, sub) }
                end
              end
            end
            ...
            def build_not_statement(ctx, stmt)
              builder = self
              ctx.not_statement do
                ctx.statement { builder.build_statement(ctx, stmt) }
              end
            end

            def build_text_transformations(ctx, transforms)
              transforms.each do |t|
                ctx.text_transformation do
