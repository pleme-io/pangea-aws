# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.

require 'pangea/resources/base'
require 'pangea/resources/reference'
require 'pangea/resources/aws_sagemaker_user_profile/types'
require 'pangea/resource_registry'

module Pangea::Resources
  module AWSSagemakerUserProfile
    include Pangea::Resources::ResourceBuilder

    define_resource :aws_sagemaker_user_profile,
      attributes_class: AWS::Types::SagemakerUserProfileAttributes,
      outputs: { id: :id },
      map: [:domain_id, :user_profile_name],
      map_present: [:single_sign_on_user_identifier, :single_sign_on_user_value, :tags, :user_settings]
  end
  module AWS
    include AWSSagemakerUserProfile
  end
end
Pangea::ResourceRegistry.register_module(Pangea::Resources::AWS)
