# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.

require 'pangea/resources/base'
require 'pangea/resources/reference'
require 'pangea/resources/aws_batch_job_definition/types'
require 'pangea/resource_registry'

module Pangea::Resources
  module AWSBatchJobDefinition
    include Pangea::Resources::ResourceBuilder

    define_resource :aws_batch_job_definition,
      attributes_class: AWS::Types::BatchJobDefinitionAttributes,
      outputs: { id: :id },
      map: [:name, :type],
      map_present: [:container_properties, :ecs_properties, :eks_properties, :node_properties, :parameters, :platform_capabilities, :retry_strategy, :scheduling_priority, :tags, :timeout],
      map_bool: [:deregister_on_new_revision, :propagate_tags]
  end
  module AWS
    include AWSBatchJobDefinition
  end
end
Pangea::ResourceRegistry.register_module(Pangea::Resources::AWS)
