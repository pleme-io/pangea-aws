# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.

require 'pangea/resources/base'
require 'pangea/resources/reference'
require 'pangea/resources/aws_elasticache_reserved_cache_node/types'
require 'pangea/resource_registry'

module Pangea::Resources
  module AWSElasticacheReservedCacheNode
    include Pangea::Resources::ResourceBuilder

    define_resource :aws_elasticache_reserved_cache_node,
      attributes_class: AWS::Types::ElasticacheReservedCacheNodeAttributes,
      outputs: { id: :id },
      map: [:reserved_cache_nodes_offering_id],
      map_present: [:cache_node_count, :region, :tags]
  end
  module AWS
    include AWSElasticacheReservedCacheNode
  end
end
Pangea::ResourceRegistry.register_module(Pangea::Resources::AWS)
