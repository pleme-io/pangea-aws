# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.

require 'pangea/resources/base'
require 'pangea/resources/reference'
require 'pangea/resources/aws_apprunner_custom_domain_association/types'
require 'pangea/resource_registry'

module Pangea::Resources
  module AWSApprunnerCustomDomainAssociation
    include Pangea::Resources::ResourceBuilder

    define_resource :aws_apprunner_custom_domain_association,
      attributes_class: AWS::Types::ApprunnerCustomDomainAssociationAttributes,
      outputs: { id: :id },
      map: [:domain_name, :service_arn],
      map_present: [:region],
      map_bool: [:enable_www_subdomain]
  end
  module AWS
    include AWSApprunnerCustomDomainAssociation
  end
end
Pangea::ResourceRegistry.register_module(Pangea::Resources::AWS)
