# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.

require 'pangea/resources/base'
require 'pangea/resources/reference'
require 'pangea/resources/aws_chime_voice_connector_origination/types'
require 'pangea/resource_registry'

module Pangea::Resources
  module AWSChimeVoiceConnectorOrigination
    include Pangea::Resources::ResourceBuilder

    define_resource :aws_chime_voice_connector_origination,
      attributes_class: AWS::Types::ChimeVoiceConnectorOriginationAttributes,
      outputs: { id: :id },
      map: [:route, :voice_connector_id],
      map_present: [:region],
      map_bool: [:disabled]
  end
  module AWS
    include AWSChimeVoiceConnectorOrigination
  end
end
Pangea::ResourceRegistry.register_module(Pangea::Resources::AWS)
