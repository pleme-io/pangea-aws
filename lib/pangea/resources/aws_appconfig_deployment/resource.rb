# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.

require 'pangea/resources/base'
require 'pangea/resources/reference'
require 'pangea/resources/aws_appconfig_deployment/types'
require 'pangea/resource_registry'

module Pangea::Resources
  module AWSAppconfigDeployment
    include Pangea::Resources::ResourceBuilder

    define_resource :aws_appconfig_deployment,
      attributes_class: AWS::Types::AppconfigDeploymentAttributes,
      outputs: { id: :id },
      map: [:application_id, :configuration_profile_id, :configuration_version, :deployment_strategy_id, :environment_id],
      map_present: [:description, :kms_key_identifier, :tags]
  end
  module AWS
    include AWSAppconfigDeployment
  end
end
Pangea::ResourceRegistry.register_module(Pangea::Resources::AWS)
