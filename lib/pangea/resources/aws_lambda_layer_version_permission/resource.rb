# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.

require 'pangea/resources/base'
require 'pangea/resources/reference'
require 'pangea/resources/aws_lambda_layer_version_permission/types'
require 'pangea/resource_registry'

module Pangea::Resources
  module AWSLambdaLayerVersionPermission
    include Pangea::Resources::ResourceBuilder

    define_resource :aws_lambda_layer_version_permission,
      attributes_class: AWS::Types::LambdaLayerVersionPermissionAttributes,
      outputs: { id: :id },
      map: [:action, :layer_name, :principal, :statement_id, :version_number],
      map_present: [:organization_id],
      map_bool: [:skip_destroy]
  end
  module AWS
    include AWSLambdaLayerVersionPermission
  end
end
Pangea::ResourceRegistry.register_module(Pangea::Resources::AWS)
