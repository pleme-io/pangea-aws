# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.

require 'pangea/resources/base'
require 'pangea/resources/reference'
require 'pangea/resources/aws_spot_fleet_request/types'
require 'pangea/resource_registry'

module Pangea::Resources
  module AWSSpotFleetRequest
    include Pangea::Resources::ResourceBuilder

    define_resource :aws_spot_fleet_request,
      attributes_class: AWS::Types::SpotFleetRequestAttributes,
      outputs: { id: :id },
      map: [:iam_fleet_role, :target_capacity],
      map_present: [:allocation_strategy, :context, :excess_capacity_termination_policy, :fleet_type, :instance_interruption_behaviour, :instance_pools_to_use_count, :launch_specification, :launch_template_config, :on_demand_allocation_strategy, :on_demand_max_total_price, :on_demand_target_capacity, :spot_maintenance_strategies, :spot_price, :tags, :target_capacity_unit_type, :terminate_instances_on_delete, :valid_from, :valid_until],
      map_bool: [:replace_unhealthy_instances, :terminate_instances_with_expiration, :wait_for_fulfillment]
  end
  module AWS
    include AWSSpotFleetRequest
  end
end
Pangea::ResourceRegistry.register_module(Pangea::Resources::AWS)
