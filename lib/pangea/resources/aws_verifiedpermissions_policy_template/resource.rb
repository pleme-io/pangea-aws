# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.

require 'pangea/resources/base'
require 'pangea/resources/reference'
require 'pangea/resources/aws_verifiedpermissions_policy_template/types'
require 'pangea/resource_registry'

module Pangea::Resources
  module AWSVerifiedpermissionsPolicyTemplate
    include Pangea::Resources::ResourceBuilder

    define_resource :aws_verifiedpermissions_policy_template,
      attributes_class: AWS::Types::VerifiedpermissionsPolicyTemplateAttributes,
      outputs: { id: :id },
      map: [:policy_store_id, :statement],
      map_present: [:description, :region]
  end
  module AWS
    include AWSVerifiedpermissionsPolicyTemplate
  end
end
Pangea::ResourceRegistry.register_module(Pangea::Resources::AWS)
