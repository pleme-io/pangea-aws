# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.

require 'pangea/resources/base'
require 'pangea/resources/reference'
require 'pangea/resources/aws_storagegateway_gateway/types'
require 'pangea/resource_registry'

module Pangea::Resources
  module AWSStoragegatewayGateway
    include Pangea::Resources::ResourceBuilder

    define_resource :aws_storagegateway_gateway,
      attributes_class: AWS::Types::StoragegatewayGatewayAttributes,
      outputs: { id: :id },
      map: [:gateway_name, :gateway_timezone],
      map_present: [:activation_key, :average_download_rate_limit_in_bits_per_sec, :average_upload_rate_limit_in_bits_per_sec, :cloudwatch_log_group_arn, :gateway_ip_address, :gateway_type, :gateway_vpc_endpoint, :maintenance_start_time, :medium_changer_type, :region, :smb_active_directory_settings, :smb_guest_password, :smb_security_strategy, :tags, :tags_all, :tape_drive_type],
      map_bool: [:smb_file_share_visibility]
  end
  module AWS
    include AWSStoragegatewayGateway
  end
end
Pangea::ResourceRegistry.register_module(Pangea::Resources::AWS)
